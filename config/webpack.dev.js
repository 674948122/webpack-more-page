...
module.exports = {
    ...
    entry: {
        ...
        hcpp_js1: './src/hcpp_js1.js',
        hcpp_js1_pc: './src/hcpp_js1_pc.js',
        ...
        hc_bsbj_zfs7: './src/hc_bsbj_zfs7.js',
        hc_bsbj_zfs7_pc: './src/hc_bsbj_zfs7_pc.js'
    },
    // 出口文件配置项
    output: {
        // 打包路径
    ...
    plugins: [
        ...
        new htmlPlugin({
            ...
            hash: true, //为了开发中js有缓存效果，所以加入hash，这样可以有效避免缓存JS。
            template:'./src/hcpp_js1.html', //是要打包的html模版路径和文件名称。
            filename: "hcpp_js1.html",
            title: "汇巢别墅装饰，22年沉淀，打造精益求精的生活",
            chunks:['hcpp_js1'], //添加引入的js,也就是entry中的key
            favicon: './favicon.ico',//添加特定的 favicon 路径到输出的 HTML 文件中。
            inject:true,//1、true或者body：所有JavaScript资源插入到body元素的底部2、head: 所有JavaScript资源插入到head元素中3、false： 所有静态资源css和JavaScript都不会注入到模板文件中
            showErrors:true,//是否将错误信息输出到html页面中
        }),
        new htmlPlugin({
            ...
            hash: true, //为了开发中js有缓存效果，所以加入hash，这样可以有效避免缓存JS。
            template:'./src/hcpp_js1_pc.html', //是要打包的html模版路径和文件名称。
            filename: "hcpp_js1_pc.html",
            title: "汇巢别墅装饰，22年沉淀，打造精益求精的生活",
            chunks:['hcpp_js1_pc'], //添加引入的js,也就是entry中的key
            favicon: './favicon.ico',//添加特定的 favicon 路径到输出的 HTML 文件中。
            inject:true,//1、true或者body：所有JavaScript资源插入到body元素的底部2、head: 所有JavaScript资源插入到head元素中3、false： 所有静态资源css和JavaScript都不会注入到模板文件中
            showErrors:true,//是否将错误信息输出到html页面中
        }),
        new htmlPlugin({
            ...
            hash: true, //为了开发中js有缓存效果，所以加入hash，这样可以有效避免缓存JS。
            template:'./src/fgh_zxfgbj_zfs6.html', //是要打包的html模版路径和文件名称。
            filename: "fgh_zxfgbj_zfs6.html",
            title: "免费算一算-装修成喜欢的风格要花多少钱",
            chunks:['fgh_zxfgbj_zfs6'], //添加引入的js,也就是entry中的key
            favicon: './favicon.ico',//添加特定的 favicon 路径到输出的 HTML 文件中。
            inject:true,//1、true或者body：所有JavaScript资源插入到body元素的底部2、head: 所有JavaScript资源插入到head元素中3、false： 所有静态资源css和JavaScript都不会注入到模板文件中
            showErrors:true,//是否将错误信息输出到html页面中
        }),
        new htmlPlugin({
            ...
            hash: true, //为了开发中js有缓存效果，所以加入hash，这样可以有效避免缓存JS。
            template:'./src/fgh_zxfgbj_zfs6_pc.html', //是要打包的html模版路径和文件名称。
            filename: "fgh_zxfgbj_zfs6_pc.html",
            title: "免费算一算-装修成喜欢的风格要花多少钱",
            chunks:['fgh_zxfgbj_zfs6_pc'], //添加引入的js,也就是entry中的key
            favicon: './favicon.ico',//添加特定的 favicon 路径到输出的 HTML 文件中。
            inject:true,//1、true或者body：所有JavaScript资源插入到body元素的底部2、head: 所有JavaScript资源插入到head元素中3、false： 所有静态资源css和JavaScript都不会注入到模板文件中
            showErrors:true,//是否将错误信息输出到html页面中
        }),
        new htmlPlugin({
            ...
            hash: true, //为了开发中js有缓存效果，所以加入hash，这样可以有效避免缓存JS。
            template:'./src/kjby_ls_0515_wkb7.html', //是要打包的html模版路径和文件名称。
            filename: "kjby_ls_0515_wkb7.html",
            title: "业之峰装饰 全国征集榜样设计",
            chunks:['kjby_ls_0515_wkb7'], //添加引入的js,也就是entry中的key
            favicon: './favicon.ico',//添加特定的 favicon 路径到输出的 HTML 文件中。
            inject:true,//1、true或者body：所有JavaScript资源插入到body元素的底部2、head: 所有JavaScript资源插入到head元素中3、false： 所有静态资源css和JavaScript都不会注入到模板文件中
            showErrors:true,//是否将错误信息输出到html页面中
        }),
        new htmlPlugin({
            ...
            hash: true, //为了开发中js有缓存效果，所以加入hash，这样可以有效避免缓存JS。
            template:'./src/kjby_ls_0515_wkb7_pc.html', //是要打包的html模版路径和文件名称。
        ...
        new htmlPlugin({
            ...
            hash: true, //为了开发中js有缓存效果，所以加入hash，这样可以有效避免缓存JS。
            template:'./src/hc_bsbj_zfs7.html', //是要打包的html模版路径和文件名称。
            filename: "hc_bsbj_zfs7.html",
            title: "别墅环保装修-免费报价",
            chunks:['hc_bsbj_zfs7'], //添加引入的js,也就是entry中的key
            favicon: './favicon.ico',//添加特定的 favicon 路径到输出的 HTML 文件中。
            inject:true,//1、true或者body：所有JavaScript资源插入到body元素的底部2、head: 所有JavaScript资源插入到head元素中3、false： 所有静态资源css和JavaScript都不会注入到模板文件中
            showErrors:true,//是否将错误信息输出到html页面中
        }),
        new htmlPlugin({
            ...
            hash: true, //为了开发中js有缓存效果，所以加入hash，这样可以有效避免缓存JS。
            template:'./src/hc_bsbj_zfs7_pc.html', //是要打包的html模版路径和文件名称。
            filename: "hc_bsbj_zfs7_pc.html",
            title: "别墅环保装修-免费报价",
            chunks:['hc_bsbj_zfs7_pc'], //添加引入的js,也就是entry中的key
            favicon: './favicon.ico',//添加特定的 favicon 路径到输出的 HTML 文件中。
            inject:true,//1、true或者body：所有JavaScript资源插入到body元素的底部2、head: 所有JavaScript资源插入到head元素中3、false： 所有静态资源css和JavaScript都不会注入到模板文件中
            showErrors:true,//是否将错误信息输出到html页面中
        }),
